...
1. Create all solid bodies (`fCuboid`, `fCylinder`, `fSphere`; the front hook rods and the hooked locking pegs are circle profiles swept along sketched paths with `opSweep`, and the helper sketches are deleted with `opDeleteBodies` immediately after each sweep) with unique ids derived from the feature id (`id + "plate"`, `id + ("peg" ~ i ~ "_" ~ j)`).
